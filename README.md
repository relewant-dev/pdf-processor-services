...
When uploaded text is classified as a CV/resume or insurance policy, the service also upserts vector DB records in the configured `candidates` or `insurances` collection. Classification and metadata extraction are performed by the configured Ollama model with prompts that expose the payload fields accepted by the vector DB schemas, so CVs and insurance documents can be written in Italian or other languages without relying on hardcoded English labels. The service fills operational fields such as `id`, `document_hash`, `raw_text`, `created_at`, and `updated_at`, while the model maps document values into fields such as `first_name`, `education`, `policy_number`, `start_date`, and `coverage_details`. Insurance PDFs are mapped to a single `insurances` record rather than one record per page or text chunk. Candidate payloads can still split long `raw_text` into chunk records for vector search. Missing optional metadata fields remain `None` or empty lists unless the extracted payload itself contains a value.
