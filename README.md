...
Candidate records support `id`, `first_name`, `last_name`, `email`, `phone`, `seniority`, `city`, `country`, `address`, `competences`, `previous_works`, `education`, `current_job_title`, `current_company`, `availability_date`, `notes`, `language`, `certifications`, `created_at`, and `updated_at`. During CV extraction, Ollama is instructed to map education sections into `education`, job/employment/work-history sections into `previous_works`, and explicit current or latest role details into `current_job_title` and `current_company`; the persistence layer also normalizes common aliases such as `academic_background`, `jobs`, `work_history`, and `current_role` into those canonical database fields. Insurance records support `id`, `candidate_id`, `policy_number`, `insurance_provider`, `insurance_type`, `policy_holder`, `coverage_details`, `start_date`, `end_date`, `premium_amount`, `currency`, `beneficiary`, `created_at`, and `updated_at`. The service also stores operational `document_hash` values to detect duplicates and uses `raw_text` only for indexing/storage, not for answer prompts.
